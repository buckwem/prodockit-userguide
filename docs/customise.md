...
Most document-wide changes are made in one of six places:

| What you are changing | File or directory |
|---|---|
| Site details, navigation, and theme | `zensical.toml` |
| PDF-only settings | `pdk-pdf.toml` |
...
| Managed website behaviour | `docs/javascripts/pdk.js` |
| Your JavaScript extensions | `docs/javascripts/extra.js` |
/// table-caption | <
...
Set a PDF-only copyright value in `pdk-pdf.toml` when its footer needs
different content:
...
`pdk.js` is managed by Prodockit and contains shared website behaviour.
`extra.js` is the user-managed extension point and is preserved by Adopt and
...
The \index{PDF!configuration} is read when you run `prodockit pdf`. PDF-only
settings live in `pdk-pdf.toml` beside `zensical.toml`. The PDF also uses the
website's rendered Markdown, site name, fonts, extensions, and most content
customisations. See
...
Set the physical page and margins in `pdk-pdf.toml`:
...
pdf-requirements.txt - PDF-only Python dependencies
